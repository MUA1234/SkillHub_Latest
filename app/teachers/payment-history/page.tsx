...
const StudentPaymentHistoryPage = () => {
  ...
  const loadPaymentData = async () => {
    try {
      ...
      const response = await apiClient.getTeacherPaymentHistory({
        search: searchQuery,
      ...
      if (response?.success) {
        paymentsData = Array.isArray(response.data?.payments) ? response.data.payments : [];
        ...
        paymentsData = response.transactions.map((t: any) => ({
      ...
      if (paymentsData.length > 0 && summaryData.totalPayments === 0) {
        summaryData = {
          totalPaid: paymentsData.filter(p => p.status === 'paid').reduce((sum, p) => sum + (p.amount || 0), 0),
          totalPending: paymentsData.filter(p => p.status === 'pending').reduce((sum, p) => sum + (p.amount || 0), 0),
          totalOverdue: paymentsData.filter(p => p.status === 'overdue').reduce((sum, p) => sum + (p.amount || 0), 0),
      ...
      <div className="flex pt-16">
        ...
        <main className="flex-1 transition-all duration-300 pt-12 lg:pt-0 p-4 sm:p-6 lg:p-8 min-h-[calc(100vh-4rem)]">
          ...
          >
            {}
            <div className="mb-8">
              <PageHeader title="Where your" accent="earnings went" />
